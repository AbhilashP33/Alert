...
def compute_dates(today: date | None = None) -> dict:
    """
    Recreates SAS macros:

    report_dt = today()
    end_dt = intnx(week.4, report_dt, 0) - 2   # Sat of current ISO-like week - 2? (matches SAS ‘week.4’)
    start_dt = (if end_dt < week_end_dt_ini then start_dt_ini else end_dt - 6)
    week_start_dt = start_dt (YYYY-MM-DD)
    week_end_dt   = end_dt   (YYYY-MM-DD)
    pardt = start_dt - 7  (YYYY-MM-DD)
    week_end_dt_p1 = end_dt + 1
    """
    if not today:
        today = date.today()
    ...
    # SAS intnx('week.4', report_dt, 0) is week ending on Thursday; then -2
    # Practical equivalent: find Thursday of current week, then minus 2 days
    # But the SAS pipeline ultimately uses week_start_dt/week_end_dt as dates for filtering;
    # We'll replicate behavior empirically as follows:
    # Find the "week.4 end" = Thursday of this week:
    weekday = today.weekday()  # Monday=0 ... Sunday=6
    # days to Thursday:
    days_to_thu = (3 - weekday) % 7
    week4_end = today + timedelta(days=days_to_thu)
    end_dt = week4_end - timedelta(days=2)  # SAS had "-2"
    ...
    return {
        "report_dt": today,                # date
        "start_dt": start_dt,
        "end_dt": end_dt,
        "week_start_dt": week_start_dt,    # date
        "week_end_dt": week_end_dt,        # date
        "week_end_dt_p1": week_end_dt_p1,  # date
        "pardt": pardt,                    # date
        "label": label,                    # yyyymmdd
        "ymd": label,
    }
...
def q_cards_dec_pref(schema: str, week_end_p1: date) -> str:
    ...
    b = f"""
        ...
        WHERE event_timestamp > TIMESTAMP '{week_end_p1.isoformat()} 00:00:00'
          ...
          AND try(json_extract_scalar({EVT_PAYLOAD}, '$.productType'))   = 'CREDIT_CARD'
    """
    return f"SELECT * FROM ({a}) UNION ALL SELECT * FROM ({b})"
...
def run_pipeline(conf: dict):
    ...
    # Dedup to one row per decisionId (SAS kept sort+nodupkey by decisionId)
    # Keep the latest by eventTimestamp (and Y first if tie)
    df_join["_rank"] = df_join.groupby("decisionid")["eventTimestamp"].rank(method="first", ascending=False)
    # Prefer rows with Y
    df_join["_prefY"] = (df_join["dec_tm_ge_pref_tm"] == "Y").astype(int)
    df_join = df_join.sort_values(by=["decisionid", "_prefY", "eventTimestamp"], ascending=[True, False, False])
    df_cards_dec_pref2 = df_join.drop_duplicates(subset=["decisionid"], keep="first").drop(columns=["_rank", "_prefY"])
    ...
    # Time_Diff in seconds (eventTimestamp_a - eventTimestamp)
    def seconds_diff(a, b):
        if pd.isna(a) or pd.isna(b):
            return math.nan
        return (a - b).total_seconds()

    df_final["Time_Diff"] = [seconds_diff(a, b) for a, b in zip(df_final["eventTimestamp_a"], df_final["eventTimestamp"])]

    # SLA_Ind = 'Y' if Time_Diff <= 1800 and not missing
    df_final["SLA_Ind"] = [ "Y" if (not pd.isna(x) and x <= 1800) else "N" for x in df_final["Time_Diff"] ]

    # Alert_Time buckets
    def alert_time_bucket(x):
        if pd.isna(x): return "99 - Timestamp is missing"
        if x < 0:      return "00 - Less than 0 seconds"
        if x <= 1800:  return "01 - Less than or equal to 30 minutes"
        if x <= 3600:  return "02 - Greater than 30 mins and less than or equal to 60 mins"
        if x <= 7200:  return "03 - Greater than 1 hour and less than or equal to 2 hours"
        if x <= 10800: return "04 - Greater than 2 hours and less than or equal to 3 hours"
        if x <= 14400: return "05 - Greater than 3 hours and less than or equal to 4 hours"
        if x <= 18000: return "06 - Greater than 4 hours and less than or equal to 5 hours"
        if x <= 21600: return "07 - Greater than 5 hours and less than or equal to 6 hours"
        if x <= 25200: return "08 - Greater than 6 hours and less than or equal to 7 hours"
        if x <= 28800: return "09 - Greater than 7 hours and less than or equal to 8 hours"
        if x <= 32400: return "10 - Greater than 8 hours and less than or equal to 9 hours"
        if x <= 36000: return "11 - Greater than 9 hours and less than or equal to 10 hours"
        if x <= 86400: return "12 - Greater than 10 hours and less than or equal to 24 hours"
        if x <= 172800:return "13 - Greater than 1 day and less than or equal to 2 days"
        if x <= 259200:return "14 - Greater than 2 days and less than or equal to 3 days"
        return "15 - Greater than 3 days"

    df_final["Alert_Time"] = [alert_time_bucket(x) for x in df_final["Time_Diff"]]
    ...
    completeness_fail = (
        ...
        .assign(
            ...
            custid_mask=lambda d: d["customerid"].fillna("").str[-3:].radd("******"),
        )[[
